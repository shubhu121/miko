...
class MainActivity : BaseNavigationActivity() {
    ...
    private lateinit var talkMascot: android.widget.ImageView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        talkMascot = findViewById(R.id.talk_mascot)
    ...
    private fun handleIntent(intent: Intent?) {
        if (intent?.action == "com.blurr.voice.WAKE_UP_PANDA") {
            Logger.d("MainActivity", "Wake up Miko shortcut activated!")
    ...
    private fun startConversationalAgent() {
        if (!ConversationalAgentService.isRunning) {
            ...
            Toast.makeText(this, "Miko is waking up...", Toast.LENGTH_SHORT).show()
        } else {
            Logger.d("MainActivity", "ConversationalAgentService is already running.")
            Toast.makeText(this, "Miko is already awake!", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun setupClickListeners() {
        ...
        // Tap the mascot to start talking to Miko.
        talkMascot.setOnClickListener {
    ...
    private fun requestLimitIncrease() {
        ...
        val recipient = "shubhu0788@gmail.com"
    ...
    private fun setupProBanner() {
        ...
        upgradeButton.setOnClickListener {
            Toast.makeText(this, "All Miko features are already unlocked.", Toast.LENGTH_SHORT).show()
        }
    ...
    private fun updateDeltaVisuals(state: PandaState) {
        runOnUiThread {
            // Gently pulse the mascot when Miko is actively working; steady otherwise.
            if (DeltaStateColorMapper.isActiveState(state)) {
                talkMascot.animate().alpha(0.5f).setDuration(600).withEndAction {
                    talkMascot.animate().alpha(1f).setDuration(600).start()
                }.start()
            } else {
                talkMascot.animate().cancel()
                talkMascot.alpha = 1f
            }
        }
    }
    ...
    private fun showDisclaimerDialog() {
        val dialog = AlertDialog.Builder(this)
            .setTitle("Disclaimer")
            .setMessage("Miko is an experimental AI assistant and is still in development. It may not always be accurate or perform as expected. It does small task better. Your understanding is appreciated!")
    ...
    private fun unlockProductAccess() {
        showLoading(false)
        tasksLeftTag.visibility = View.GONE
        proSubscriptionTag.visibility = View.GONE
        findViewById<View>(R.id.pro_upgrade_banner)?.visibility = View.GONE
    }
