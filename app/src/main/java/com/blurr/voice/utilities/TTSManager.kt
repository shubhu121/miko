...
import com.blurr.voice.api.DeepgramTts
...
class TTSManager private constructor(private val context: Context) : TextToSpeech.OnInitListener {
    ...
    private var ttsPlaybackDeferred: CompletableDeferred<Unit>? = null
    ...
    private fun setupAudioTrack() {
        ...
        audioTrack?.setPlaybackPositionUpdateListener(object : AudioTrack.OnPlaybackPositionUpdateListener {
            override fun onMarkerReached(track: AudioTrack?) {
                ttsPlaybackDeferred?.complete(Unit)
            }
    ...
    fun stop() {
        ...
        if (ttsPlaybackDeferred?.isActive == true) {
            ttsPlaybackDeferred?.completeExceptionally(CancellationException("Playback stopped by new request."))
        }
    }
    ...
    private suspend fun playWithSmartQueue(textChunks: List<String>, selectedVoice: TTSVoice) {
        ...
        val firstAudioData = getCachedAudio(firstChunk, selectedVoice) ?: try {
            DeepgramTts.synthesize(firstChunk, selectedVoice).also { audioData ->
        ...
        val preloadJob = CoroutineScope(Dispatchers.IO).launch {
            for (i in 1 until textChunks.size) {
                ...
                if (chunk.isNotEmpty()) {
                    try {
                        val audioData = getCachedAudio(chunk, selectedVoice) ?: DeepgramTts.synthesize(chunk, selectedVoice).also { audioData ->
        ...
        while (true) {
            ...
            try {
                ...
                ttsPlaybackDeferred = CompletableDeferred()
    ...
    suspend fun playAudioData(audioData: ByteArray) {
        try {
            ...
            withTimeoutOrNull(30000) { ttsPlaybackDeferred?.await() }
