...
//                // 1. Parse the raw XML into a structured list.
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent) {
        if (event.eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            ...
            if (!packageName.isNullOrBlank() && !className.isNullOrBlank()) {
                ...
                // Feed a foreground-app change into Miko's event stream (once per app switch).
                if (packageName != lastForegroundPackage && packageName != this.packageName) {
                    lastForegroundPackage = packageName
                    try {
                        com.blurr.voice.core.events.EventBus.publish(
                            com.blurr.voice.core.events.MikoEvent.AppOpened(packageName)
                        )
                    } catch (e: Exception) {
                        Log.w("AccessibilityService", "Failed to publish AppOpened: ${e.message}")
                    }
                }
            }
        }
    ...
    /** Last foreground package emitted as an AppOpened event (de-dupes rapid re-fires). */
