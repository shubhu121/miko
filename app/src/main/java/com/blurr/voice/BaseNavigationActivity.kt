...
import com.blurr.voice.core.ui.tick
...
abstract class BaseNavigationActivity : AppCompatActivity() {
    ...
    enum class NavItem {
        HOME, TRIGGERS, MOMENTS, MEMORY, SETTINGS
    }
        ...
        findViewById<LinearLayout>(R.id.nav_home).apply {
            setOnClickListener {
                if (currentItem != NavItem.HOME) {
                    navigateToActivity(MikoHomeActivity::class.java, currentItem)
                }
            }
            alpha = if (currentItem == NavItem.HOME) 1.0f else 0.7f
        }
        
        findViewById<LinearLayout>(R.id.nav_memory).apply {
            setOnClickListener {
                if (currentItem != NavItem.MEMORY) {
                    tick()
                    startActivity(Intent(this@BaseNavigationActivity, MemoriesActivity::class.java))
                }
            }
            alpha = if (currentItem == NavItem.MEMORY) 1.0f else 0.7f
        }
    ...
    private fun navigateToActivity(activityClass: Class<*>, currentItem: NavItem) {
        findViewById<View>(R.id.content_container)?.tick()
