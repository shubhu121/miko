...
import com.blurr.voice.api.TavilyApi
...
//            }
            ...
            is Action.WebSearch -> {
                // Live web lookup via Tavily; results are fed back so the agent can reason,
                // then speak or save them.
                val results = runBlocking {
                    TavilyApi(BuildConfig.TAVILY_API).searchText(action.query)
                }
                ActionResult(
                    longTermMemory = "Searched the web for '${action.query}'.",
...
//                        finger.longPress(center.first, center.second)
            ...
            is Action.Done -> {
                // Speak the final answer to the user (Deepgram TTS) instead of only
                // overlaying/logging it — this is the agent's spoken reply.
                if (action.text.isNotBlank()) {
                    runBlocking {
                        SpeechCoordinator.getInstance(context).speakToUser(action.text)
                    }
                }
                ActionResult(
                    isDone = true,
