...
sealed class Action {
    ...
    companion object {
        ...
        private val allSpecs: Map<String, Spec> = mapOf(
            ...
            "web_search" to Spec(
                name = "web_search",
                description = "Search the web and get a concise text answer back (no browser). Use for facts, news, prices, definitions, or anything not visible on the current screen. Returns text you can then speak or save — prefer this over opening a browser when the user just wants an answer.",
                params = listOf(ParamSpec("query", String::class, "The web search query.")),
                build = { args -> WebSearch(args["query"] as String) }
            ),
