...
object UserMessageBuilder {
    ...
    private fun buildAgentStateBlock(args: Args): String {
        ...
        return buildString {
            ...
            // Miko's normalized device context (foreground app, battery, connectivity,
            // clipboard, time of day) so the agent can reason with real-world awareness.
            val deviceContext = runCatching {
                com.blurr.voice.core.Miko.context.snapshot().toPromptString()
            }.getOrNull()
