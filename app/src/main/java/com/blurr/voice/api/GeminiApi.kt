...
object GeminiApi {
    ...
    suspend fun generateContent(
        ...
        while (attempts < maxRetry) {
            ...
            try {
                ...
                // DIRECT MODE: when no Cloud Function proxy is configured, call Gemini
                // straight from the device using the rotated GEMINI_API_KEYS. Mirrors the
                // dispatcher in v2/llm/GeminiAPI.kt. (Images are ignored, as in proxy mode.)
                if (proxyUrl.isBlank() || proxyKey.isBlank()) {
                    val responseBody = performDirectGeminiCall(chat, modelName, currentApiKey)
                    val directRequestTime = System.currentTimeMillis() - requestStartTime
                    val directTotalTime = System.currentTimeMillis() - attemptStartTime
                    val directLog = createLogEntry(
                        attempt = attempts + 1,
                        modelName = modelName,
                        ...
                        responseTime = directRequestTime,
                        totalTime = directTotalTime
                    )
                    saveLogToFile(MyApplication.appContext, directLog)
                    return responseBody
                }
    ...
    /**
     * DIRECT MODE: calls the Gemini model straight from the device via the official
     * Google AI SDK, using the caller-rotated [apiKey]. Used as a fallback when no
     * secure proxy (GCLOUD_PROXY_URL / GCLOUD_PROXY_URL_KEY) is configured. Text-only,
     * matching the proxy payload which also ignores images. Returns the model's text
     * or throws so the existing retry/backoff loop handles the failure.
     */
    private suspend fun performDirectGeminiCall(
        chat: List<Pair<String, List<Any>>>,
        modelName: String,
        apiKey: String
    ): String {
        ...
        val history = chat.map { (role, parts) ->
            val sdkRole = if (role.equals("model", ignoreCase = true)) "model" else "user"
            content(sdkRole) {
                parts.filterIsInstance<TextPart>().forEach { text(it.text) }
            }
        }
        val response = model.generateContent(*history.toTypedArray())
        return response.text
            ?: throw Exception(
                "Blocked or empty response from Gemini. Reason: " +
                    (response.promptFeedback?.blockReason?.name ?: "UNKNOWN")
            )
    }
