...
//        }
    ...
    /** Lets the user dictate the trigger instruction instead of typing it. */
    private fun setupVoiceInput() {
        val voiceButton = findViewById<Button>(R.id.voiceInputButton)
        val idle = "🎙  Speak instruction"
        ...
        voiceButton.setOnClickListener {
            ...
            lifecycleScope.launch {
                SpeechCoordinator.getInstance(this@CreateTriggerActivity).startListening(
                    onResult = { text ->
                        runOnUiThread {
                            if (text.isNotBlank()) {
                                val existing = instructionEditText.text?.toString().orEmpty()
                                instructionEditText.setText(if (existing.isBlank()) text else "$existing $text")
                                instructionEditText.setSelection(instructionEditText.text.length)
                            }
                            voiceButton.text = idle
                        }
                    },
                    onError = { err ->
                        runOnUiThread {
                            Toast.makeText(this@CreateTriggerActivity, "Didn't catch that ($err).", Toast.LENGTH_SHORT).show()
                            voiceButton.text = idle
                        }
                    },
                    onListeningStateChange = { isListening ->
                        runOnUiThread { voiceButton.text = if (isListening) listening else idle }
                    },
                    onPartialResult = { partial ->
                        runOnUiThread { if (partial.isNotBlank()) instructionEditText.hint = partial }
                    }
                )
            }
        }
    ...
    private fun showExactAlarmPermissionDialog() {
        val dialog = androidx.appcompat.app.AlertDialog.Builder(this)
            ...
            .setMessage("To schedule tasks at a precise time, Miko needs the 'Alarms & Reminders' permission. Please grant this in the next screen.")
